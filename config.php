...
// Auth Helpers
// ==============================
function is_logged_in(){ return !empty($_SESSION['uid']) && isset($_SESSION['tgid']); }
...
function require_admin(){ if(!is_admin()){ header('Location: /index.php'); exit; } }
...
function upsert_user_from_tg($u){
    ...
    if($row){
        $st = $pdo->prepare('UPDATE users SET username=:username, first_name=:first_name, last_name=:last_name, photo_url=:photo_url, auth_date=:auth_date, last_login_at=NOW() WHERE telegram_id=:tg');
    }else{
        $st = $pdo->prepare('INSERT INTO users (telegram_id, username, first_name, last_name, photo_url, auth_date, last_login_at) VALUES (:tg,:username,:first_name,:last_name,:photo_url,:auth_date,NOW())');
    }
